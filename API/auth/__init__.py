...
def get_optional_user(access_token: str = Header(default=None)) -> AuthUser:
    """
    Get the user's access token, which is optional

    Args:
        access_token (str, optional): The access token obtained from the request header

    Returns:
        AuthUser: An AuthUser object representing the authenticated user if an access token is provided,
        otherwise, return a guest user with limited options.
